...
export default function ChatPanel({ contextText, contextLabel, systemInstruction, onApply, onClose, settings }: ChatPanelProps) {
  ...
  const renderContent = (content: string) => {
    ...
    return parts.map((part, i) => (
      <span key={i}>
        {part}
        {i < parts.length - 1 && (
          <span className="inline-block bg-cd-accent/20 text-cd-accent text-xs px-2 py-0.5 rounded mt-1">
  ...
  return (
    <div className="w-80 border-l border-white/5 bg-cd-card flex flex-col h-full shrink-0">
      ...
      {/* Context preview */}
      <div className="px-4 py-2 border-b border-white/5 bg-cd-bg-secondary">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="text-center text-xs text-cd-subtle mt-8">
          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] rounded-xl px-3 py-2 text-sm ${
              msg.role === 'user'
                ...
                : 'bg-cd-bg-secondary text-cd-text'
            }`}>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-cd-bg-secondary rounded-xl px-3 py-2">
              <div className="flex gap-1">
                <div className="w-1.5 h-1.5 bg-cd-subtle rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <div className="w-1.5 h-1.5 bg-cd-subtle rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <div className="w-1.5 h-1.5 bg-cd-subtle rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
            </div>
          </div>
        )}
      ...
      <div className="p-3 border-t border-white/5">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="self-end p-2 rounded-lg bg-cd-accent text-white hover:bg-cd-accent/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-visible:ring-2 focus-visible:ring-cd-accent/50"
          >
